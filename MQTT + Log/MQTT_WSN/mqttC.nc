...
module mqttC {
  ...
  task void sendSubscribe() {
	...
	if(TOS_NODE_ID<4){ //From node 2 to 3, we define a high QOS on all topics, this choice is an arbitrary one
		mess->qos[TEMPERATURE-1] = QOS_HIGH;
		mess->qos[HUMIDITY-1] = QOS_HIGH;
	} else { //From node 4 to 5, we define a low QOS on all topics, this choice is an arbitrary one
  ...
  event void Read.readDone(error_t result, uint16_t data) {
	...
	if(TOS_NODE_ID<4){ //from node 2 to 3	
		mess->qos = QOS_HIGH;
	} else { //from node 4 to 5 
		mess->qos = QOS_LOW;
